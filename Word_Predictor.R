...
predict0 <-function(input,profanity,unigramDF, bigramDF, trigramDF, maxResults = 3) {
  ...
  input <- tolower(input)
  if(input == ''|input == "blah blah") return('Warning: Just input something')
...
predictKN2 <- function(input, profanity,D2,P,subbi,cw2,nw2,unigram,bigram,maxResults = 3){
...
predictKN <- function(input, profanity,unigramDF,bigramDF,trigramDF,maxResults = 3){
  ...
  if(cw1w2 == 0){
    ...
    return(predictKN2(input2, profanity,D2,p1,subbi,cw2,nw2,unigramDF,bigramDF,maxResults = maxResults))
  }
  cp <- unique(subbi$name)
  pkn <- rep(NA,length(cp))
  for(i in 1:length(cp)){
